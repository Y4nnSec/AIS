...
1. **Confidentialité** : chiffrer le flux HTTP avec **TLS** pour que personne sur puisse lire les flux en clair
1. **Authenticité** : prouver que le serveur est bien celui qu’il prétend être — via un **certificat X.509** signé par une Autorité de Certification de confiance.
1. **Intégrité** : TLS garantit aussi que les données n’ont pas été modifiées en transit.
1. **Renforcement** : configurer TLS correctement, activer HSTS, cookies sécurisés, et en complément des headers de sécurité.
